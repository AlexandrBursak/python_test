...
# MMM: Gibberish (Python 3)
text = """may out come come at your him when be down like people and from which is now have were at that one but would by other the other
...
def analize_data(data):
    ...
    for i, line in enumerate(lines):
        ...
        for word in worlds:
            dictionary[word] += 1
            one_line_text.append(word)
        unique_words = set(worlds)
        count_unique_words = len(unique_words)
        storage[i] = [len(line), len(worlds), count_unique_words, unique_words]

# What is the most common word in this text?
def get_most_popular_word():
    print('What is the most common word in this text?')
    return max(dictionary.items(), key=lambda k: k[1])

# What are the 10 most common words in this text, and how often does each appear?
def get_10_most_popular():
    print('What are the 10 most common words in this text, and how often does each appear?')
    new_dict = sorted(dictionary.items(), key=lambda k: k[1])
    return new_dict[:-10:-1]

# Which line number (where the first line is line 0) is the longest?
def get_longest_line():
    print('Which line number (where the first line is line 0) is the longest?')
    longest_line = max(storage.items(), key=lambda k: k[1])
    # print('longest_line, long, count_words, words', longest_line)
    return longest_line[0]

# Which line number (where the first line is line 0) has the most unique words?
def get_most_unique_line():
    print('Which line number (where the first line is line 0) has the most unique words?')
    heap = []
    for line, (_, _, count_uniq, _) in storage.items():
        heapq.heappush(heap, (-count_uniq, line))
    return heapq.heappop(heap)

# How many unique words in this file are palindromes (the same forwards and backwards)?
def count_polindroms():
    print('How many unique words in this file are palindromes (the same forwards and backwards)?')
    counter = 0
...
# Think of all the text as one consecutive body, with newlines used only for formatting (so the text flows from line to line). What is the most common word bi-gram (two consecutive words)? For example, the bigram “I said” appears only one time.
def get_longest_repeated_phrase():
    print('Think of all the text as one consecutive body, with newlines used only for formatting (so the text flows from line to line). What is the most common word bi-gram (two consecutive words)? For example, the bigram “I said” appears only one time.')
    bigrams = list(zip(one_line_text, one_line_text[1:]))
    count_by_bigrams = Counter(bigrams)
    result = count_by_bigrams.most_common(1)
    return result

#Restructure the text such that each line fills at most 80 characters. Fill as many of the 80 characters as possible before going to the next line, and do not break words over lines. How many lines does this restructured text fill?
def line_wrap_by_80_characters():
    print('Restructure the text such that each line fills at most 80 characters. Fill as many of the 80 characters as possible before going to the next line, and do not break words over lines. How many lines does this restructured text fill?')
    lines = ['']
    idx = 0
    for word in one_line_text:
        new_word = word + ' '
        if (len(new_word) + len(lines[idx])) > 80:
            idx += 1
            lines.append('')
        lines[idx] += new_word
...
# for line, [lenght_line, count_words, count_uniq, words] in storage.items():
#     print(line, ':', lenght_line, count_words, count_uniq, words)
# for word, count in dictionary.items():
#     print(word, ':', count)

print(get_most_popular_word())
print(get_10_most_popular())
print(get_longest_line())
print(get_most_unique_line())
print(count_polindroms())
print(get_longest_repeated_phrase())
print(line_wrap_by_80_characters())
